...
import json #json data read
import os
import time
import datetime
from streamlit_cookies_manager import CookieManager #session
from sentence_transformers import SentenceTransformer #skill
from sklearn.feature_extraction.text import TfidfVectorizer #convert to machince readable form
from sklearn.cluster import KMeans #clusters for trending skills
import torch
from torch.nn.functional import cosine_similarity #Skill matching
...
# ---- Paths ----
DATA_DIR = "C:\\Users\\HARMISH\\Desktop\\AI - Driven Skill Based\\AI - Driven Skill Based\\data"
RESUME_DIR = "C:\\Users\\HARMISH\\Desktop\\AI - Driven Skill Based\\AI - Driven Skill Based\\resumes"
    ...
    if employer_jobs.empty:
        ...
        for idx, job in employer_jobs.iterrows():
            with st.expander(f"{job['Title']} at {job['Company']}"):
                edited_job = st.text_input("Job Title", job['Title'],key=f"title{idx}")
                edited_location = st.text_input("Location", job['Location'],key=f"location{idx}")
                edited_experience = st.number_input("Experience Required", min_value=0, value=job['Experience'],key=f"exp{idx}")
                edited_skills = st.text_area("Skills", job['Skills'],key=f"skill{idx}")
                edited_salary = st.text_input("Salary", job['Salary'],key=f"salary{idx}")
                edited_type = st.selectbox("Job Type", ["Full-Time", "Part-Time", "Internship", "Contract"], index=["Full-Time", "Part-Time", "Internship", "Contract"].index(job['Type']),key=f"type{idx}")
                if st.button(f"Update {job['Title']}",key=f"update{idx}"):
                ...
                if st.button(f"Delete {job['Title']}",key=f"delete{idx}"):
                    df = df.drop(idx)
                    df.to_csv(JOB_CSV, index=False)
